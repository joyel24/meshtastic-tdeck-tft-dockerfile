...
RUN bash -c "python3 -m venv meshtastic && cd meshtastic && source bin/activate && git clone --recursive https://github.com/meshtastic/firmware.git && cd firmware && git submodule update --init --recursive --remote && cd ../../lib/ && git clone --recursive https://github.com/meshtastic/device-ui.git && git pull origin master && git checkout master && cd ../.. && /root/.platformio/penv/bin/pio run -e t-deck-tft"
